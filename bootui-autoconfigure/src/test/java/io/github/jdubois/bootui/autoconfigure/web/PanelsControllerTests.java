...
class PanelsControllerTests {
    ...
    void panelsMarksAlwaysAvailablePanelsAsAvailable() throws Exception {
        try (GenericApplicationContext context = new GenericApplicationContext()) {
            ...
            mvc.perform(get("/bootui/api/panels"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath(panelPath(BootUiPanels.OVERVIEW) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.MEMORY) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.TUNING_ADVISOR) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.HEAP_DUMP) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.THREADS) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.CONFIG) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.PENTEST) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.TRACES) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.HTTP_PROBE) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.VULNERABILITIES) + ".available").value(true));
        }
    }
    ...
    void panelsMarksActuatorBackedPanelsUnavailableWhenEndpointsAreAbsent() throws Exception {
        try (GenericApplicationContext context = new GenericApplicationContext()) {
            ...
            mvc.perform(get("/bootui/api/panels"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath(panelPath(BootUiPanels.HEALTH) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.METRICS) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.STARTUP) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.LOGGERS) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.BEANS) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.CONDITIONS) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.MAPPINGS) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.SECURITY_LOGS) + ".available").value(false))
                    ...
                    .andExpect(jsonPath(panelPath(BootUiPanels.HTTP_EXCHANGES) + ".available").value(false))
    ...
    void panelsMarksTelemetryPanelsUnavailableWhenTelemetryIsDisabled() throws Exception {
        try (GenericApplicationContext context = new GenericApplicationContext()) {
            ...
            mvc.perform(get("/bootui/api/panels"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath(panelPath(BootUiPanels.AI) + ".available").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.AI) + ".unavailableReason")
                            .value("Telemetry receiver is disabled"))
                    .andExpect(jsonPath(panelPath(BootUiPanels.TRACES) + ".available").value(false))
    ...
    void panelsExposeEnabledAndReadOnlyState() throws Exception {
        try (GenericApplicationContext context = new GenericApplicationContext()) {
            ...
            mvc.perform(get("/bootui/api/panels"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath(panelPath(BootUiPanels.CONFIG) + ".enabled").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.CONFIG) + ".available").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.CONFIG) + ".readOnly").value(false))
                    .andExpect(jsonPath(panelPath(BootUiPanels.LOGGERS) + ".enabled").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.LOGGERS) + ".readOnly").value(true))
                    ...
                    .andExpect(jsonPath(panelPath(BootUiPanels.PENTEST) + ".enabled").value(true))
                    .andExpect(jsonPath(panelPath(BootUiPanels.PENTEST) + ".readOnly").value(true))
