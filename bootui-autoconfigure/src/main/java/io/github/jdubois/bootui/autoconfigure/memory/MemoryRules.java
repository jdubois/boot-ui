...
final class CodeCacheSaturationRule extends AbstractMemoryRule {
    ...
    io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluateRule(MemoryContext context) {
        ...
        if (pool.usedPercent() >= THRESHOLD_PERCENT) {
            return violation("Code cache is " + pool.usedPercent() + "% full (" + MemoryFormat.bytes(pool.used())
                    + " of " + MemoryFormat.bytes(pool.max()) + ").");
        }
        return pass();
    }
...
final class DirectBufferGrowthRule extends AbstractMemoryRule {
    ...
    private static final long ABSOLUTE_WARN_BYTES = 256L * MemoryFormat.MEGABYTE;
    ...
    io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluateRule(MemoryContext context) {
        ...
        if (max > 0 && used >= (long) (max * 0.8)) {
            int percent = MemoryFormat.percentOf(used, max);
            return violation("Direct buffers use " + MemoryFormat.bytes(used) + " (" + percent
                    + "% of -XX:MaxDirectMemorySize " + MemoryFormat.bytes(max) + ") across "
                    + memory.directBufferCount() + " buffers.");
        }
        if (used >= ABSOLUTE_WARN_BYTES) {
            return violation("Direct buffer pool holds " + MemoryFormat.bytes(used) + " of off-heap memory across "
...
final class GcChoiceVsHeapSizeRule extends AbstractMemoryRule {

    private static final long LARGE_HEAP_BYTES = 4L * MemoryFormat.GIGABYTE;
    private static final long SMALL_HEAP_BYTES = 2L * MemoryFormat.GIGABYTE;
    ...
    io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluateRule(MemoryContext context) {
        ...
        if (memory.heapMax() >= LARGE_HEAP_BYTES && g1 && !lowLatency) {
            return violation("Heap max is " + MemoryFormat.bytes(memory.heapMax())
...
final class CollectionBloatRule extends AbstractMemoryRule {
    ...
    io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluateRule(MemoryContext context) {
        ...
        for (HeapClassHistogramEntryDto entry : candidates.subList(0, Math.min(MAX_REPORTED, candidates.size()))) {
            int sharePercent = MemoryFormat.percentOf(entry.bytes(), totalBytes);
            details.add(entry.className() + " retains " + MemoryFormat.bytes(entry.bytes()) + " (" + sharePercent
    ...
    io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluateRule(MemoryContext context) {
        ...
        return context.heapContent().histogram().stream()
                ...
                .map(entry -> violation(entry.className() + " retains "
                        + MemoryFormat.percentOf(entry.bytes(), totalBytes) + "% of the sampled heap ("
