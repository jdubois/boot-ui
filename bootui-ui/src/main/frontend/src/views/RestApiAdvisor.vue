...
async function loadReport() {
  try {
    const res = await apiFetch('api/rest-advisor')
...
async function runScan() {
  ...
  loading.value = true
  try {
    const res = await apiFetch('api/rest-advisor/scan', {method: 'POST'})
    if (!res.ok) throw new Error('HTTP ' + res.status)
    report.value = await res.json()
    error.value = null
  } catch (e) {
          ...
          <div>
            ...
            <div class="text-muted small">
              <template v-if="hasScanData && report.violationsFound > 0">
                {{ report.violationsFound }} {{ pluralize(report.violationsFound, 'flagged rule') }}, sorted by
                importance
              </template>
              <template v-else>{{ report.violationsFound }} rule finding(s)</template>
            </div>
          </div>
