...
import RestApiAdvisor from './RestApiAdvisor.vue'
...
async function mountWithReport(report) {
  ...
  const wrapper = mount(RestApiAdvisor)
  await flushPromises()
  return wrapper
}

describe('RestApiAdvisor', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })
  ...
  it('shows an empty findings state when every evaluated rule passes', async () => {
    const wrapper = await mountWithReport(
      restAdvisorReport([ruleResult('RAPI-DOC-001', 'Passing informational rule', 'INFO', 'PASS')], 0)
    )
