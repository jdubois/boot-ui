...
read-only. The complete list is in the [property reference](./properties).
...
violating rules, sorted by severity and violation count. See the
[architecture checks catalogue](https://github.com/jdubois/boot-ui/blob/main/docs/ARCHITECTURE-CHECKS.md) for the full
list of rules and what each one inspects.
...
checks can be added without expanding the scanner's HTTP surface. See the
[pentest checks catalogue](https://github.com/jdubois/boot-ui/blob/main/docs/PENTEST-CHECKS.md) for the full list of
checks and what each one inspects.
...
::: warning Masking scope
BootUI masks discovered *connection details* (for example credentials embedded in a JDBC URL or
