...
abstract class AbstractMemoryRule implements MemoryRule {
    ...
    abstract MemoryEvaluation evaluateRule(MemoryContext context);

    @Override
    public final io.github.jdubois.bootui.core.dto.MemoryRuleResultDto evaluate(MemoryContext context) {
        return evaluateWithEvidence(context).result();
    }

    @Override
    public final MemoryEvaluation evaluateWithEvidence(MemoryContext context) {
        try {
            if (definition.category() == MemoryCategory.THREADS
                    && context.threads().collectionError() != null) {
                return MemoryEvaluation.unknown(
                        MemoryRuleSupport.error(definition, context.threads().collectionError()));
            }
            if (definition.category() == MemoryCategory.HEAP_CONTENT
                    && context.heapContent().collectionError() != null) {
                return MemoryEvaluation.unknown(MemoryRuleSupport.error(
                        definition, context.heapContent().collectionError()));
            }
            return evaluateRule(context);
        } catch (RuntimeException | LinkageError ex) {
            return MemoryEvaluation.unknown(
                    MemoryRuleSupport.error(definition, "Rule could not be evaluated: " + ex.getMessage()));
        }
    }

    MemoryEvaluation pass() {
        return MemoryEvaluation.complete(MemoryRuleSupport.pass(definition));
    }

    MemoryEvaluation skipped(String reason) {
        return MemoryEvaluation.unknown(MemoryRuleSupport.skipped(definition, reason));
    }

    MemoryEvaluation inapplicable(String reason) {
        return MemoryEvaluation.inapplicable(MemoryRuleSupport.skipped(definition, reason));
    }

    MemoryEvaluation noTargets() {
        return MemoryEvaluation.inapplicable(MemoryRuleSupport.pass(definition));
    }

    MemoryEvaluation missingPass() {
    ...
    HighHeapUtilizationRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        "MEDIUM",
                        "Flags a heap-occupancy snapshot at least 95% of the reported maximum. A post-histogram"
                                + " reading is preferred, but histogram success does not verify a completed full GC and"
                                + " allocations can resume before the reading. Occupancy can include reclaimable garbage"
                                + " and is not a retained live set.",
                        "Confirm pressure under representative load with GC logs or a profiler before reducing"
                                + " retention or raising -Xmx; check total process and container headroom first.",
        ...
        if (percent >= THRESHOLD_PERCENT) {
            return violation(
                    ...
                    "Heap is " + percent + "% full (" + MemoryFormat.bytes(memory.heapUsed()) + " of "
                            + MemoryFormat.bytes(memory.heapMax())
                            + ") in a single snapshot that may include not-yet-collected garbage; confirm it persists"
                            + " after a GC.");
        }
        return pass();
    }
...
final class OldGenerationNearMaxRule extends AbstractMemoryRule {
    ...
    OldGenerationNearMaxRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.HEAP_PRESSURE,
                "MEDIUM",
                "Flags old-generation occupancy at 85% of a reported pool maximum. The post-histogram snapshot is"
                        + " preferred, but does not prove a completed full GC or long-lived retention. Pool boundaries and"
                        + " reclamation differ between collectors.",
                "Confirm pressure with collector-specific GC logs and representative workload observations before"
                        + " changing heap or generation sizing.",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (oldGen.isEmpty()) {
            return inapplicable("No old-generation memory pool is exposed by the active garbage collector.");
        }
...
final class SmallMaxHeapUnderPressureRule extends AbstractMemoryRule {
    ...
    SmallMaxHeapUnderPressureRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.HEAP_PRESSURE,
                "LOW",
                "Flags high heap occupancy with a maximum below 15% of a container limit of at least 1 GiB. A"
                        + " container limit is not available free memory: other processes and native allocations can"
                        + " consume the remaining capacity.",
                "Confirm sustained heap pressure and actual container/native headroom before raising -Xmx or"
                        + " -XX:MaxRAMPercentage.",
...
final class MetaspaceSaturationRule extends AbstractMemoryRule {
    ...
    MetaspaceSaturationRule() {
        super(new MemoryRuleDefinition(
                ...
                "MEDIUM",
                "Flags when the Metaspace pool is nearly full, which can cause OutOfMemoryError: Metaspace, often from"
                        + " classloader leaks or excessive dynamic class generation.",
                "Raise -XX:MaxMetaspaceSize, or investigate classloader leaks and runtime class generation (proxies,"
                        + " scripting).",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (metaspace.isEmpty()) {
            return inapplicable("No Metaspace pool is exposed by this JVM.");
        }
        if (metaspace.get().max() <= 0) {
            return inapplicable("Metaspace has no configured maximum (effectively unbounded).");
        }
...
final class DirectBufferGrowthRule extends AbstractMemoryRule {
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (capacity < 0 || max <= 0) {
            return skipped(
                    "Direct-buffer capacity or its effective maximum is unavailable; a missing cap is not an unlimited"
                            + " cap.");
        }
...
final class MissingHeapSizingInContainerRule extends AbstractMemoryRule {
    ...
    MissingHeapSizingInContainerRule() {
        super(new MemoryRuleDefinition(
                ...
                "INFO",
                "Notes a detected container memory limit with neither -Xmx/-XX:MaxHeapSize nor an explicit RAM-sizing"
                        + " option set. The JVM is container-aware and normally defaults the max heap to about 25% of the"
                        + " limit, while small-heap ergonomics can use 50%.",
                "Set -XX:MaxRAMPercentage (or an explicit -Xmx/-XX:MaxHeapSize) if you want the heap sized"
                        + " deliberately rather than by default ergonomics.",
                "https://docs.oracle.com/en/java/javase/21/docs/specs/man/java.html"));
    }
...
final class ContainerSupportDisabledRule extends AbstractMemoryRule {
    ...
    ContainerSupportDisabledRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.GC_CONFIGURATION,
                "HIGH",
                "Detects -XX:-UseContainerSupport while a cgroup memory limit is visible. Container support is enabled"
                        + " by default on supported JDKs; disabling it makes JVM ergonomics use host-level memory and CPU"
                        + " information instead of container constraints, which can oversize the heap and JVM worker"
                        + " pools.",
                "Remove -XX:-UseContainerSupport so heap, GC, JIT, and common-pool ergonomics respect the container"
                        + " limits; use explicit -Xmx/-XX:MaxRAMPercentage and -XX:ActiveProcessorCount only when"
                        + " deliberate overrides are required.",
                "https://docs.oracle.com/en/java/javase/21/docs/specs/man/java.html"));
    }
    ...
    DeadlockDetectedRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        "CRITICAL",
                        "Detects platform threads blocked in a cycle of lock acquisition; deadlocked threads make no"
                                + " progress and can hang request processing.",
                        "Inspect the deadlocked threads in the Threads panel, then establish a consistent global"
                                + " lock-ordering or use tryLock with timeouts.",
...
final class HighBlockedThreadRatioRule extends AbstractMemoryRule {
    ...
    HighBlockedThreadRatioRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.THREADS,
                "MEDIUM",
                "Flags when a large share of live threads are BLOCKED waiting for monitors, indicating lock contention"
                        + " that limits throughput. Two trigger paths: (1) ratio path — at least 5 BLOCKED threads and"
                        + " >=25% of all live threads; (2) absolute path — at least 20 BLOCKED threads and >=10% of all"
                        + " live threads (prevents false positives in large pools). Both paths apply to a single snapshot,"
                        + " so a transient burst can trigger the rule; confirm the finding persists before acting.",
                "Identify the contended lock in the Threads panel and reduce the critical section, shard the lock, or"
...
final class RunawayCpuThreadRule extends AbstractMemoryRule {
    ...
    RunawayCpuThreadRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        MemoryCategory.THREADS,
                        "INFO",
                        "Highlights RUNNABLE platform threads whose accumulated CPU time is a large fraction of the"
                                + " JVM's uptime, i.e. they have kept a core busy for much of the process's life. CPU time"
                                + " is cumulative since the thread started, so this is a hot-loop candidate to"
                                + " investigate, not a confirmed problem. The rule skips when its bounded platform-thread"
                                + " detail page is incomplete.",
                        "Correlate with two consecutive thread snapshots; if CPU keeps climbing for the same thread,"
                                + " profile its stack for a hot or spinning loop.",
                        "https://docs.oracle.com/en/java/javase/21/docs/api/java.management/java/lang/management/ThreadMXBean.html"));
    }
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        boolean missingCpu = threads.threads().isEmpty();
        for (ThreadInfoDto thread : threads.threads()) {
            if (thread.cpuTimeMillis() == null || thread.cpuTimeMillis() < 0) missingCpu = true;
        ...
        if (hot.isEmpty()) {
            return missingCpu ? missingPass() : pass();
        }
...
final class BigObjectsRule extends AbstractMemoryRule {
    ...
    BigObjectsRule() {
        super(new MemoryRuleDefinition(
                ...
                "INFO",
                "Surfaces classes whose average shallow size per instance is large; these big objects dominate"
                        + " allocation, can become G1 humongous allocations, and may fragment the heap.",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        for (HeapClassHistogramEntryDto entry : context.heapContent().histogram()) {
            if (entry.instances() > 0 && entry.bytes() >= 0) observed++;
        ...
        if (candidates.isEmpty()) {
            return observed == 0 ? noTargets() : pass();
        }
...
final class CollectionBloatRule extends AbstractMemoryRule {
    ...
    CollectionBloatRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.HEAP_CONTENT,
                "MEDIUM",
                "Flags JDK collection or map classes (including their node/entry backing structures) that occupy a"
...
final class DominantClassRule extends AbstractMemoryRule {
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (context.heapContent().histogram().stream().noneMatch(entry -> !isArrayClass(entry.className()))) {
            return noTargets();
        }
...
final class ExcessiveLoadedClassesRule extends AbstractMemoryRule {
    ...
    ExcessiveLoadedClassesRule() {
        super(new MemoryRuleDefinition(
                ...
                "INFO",
                "Reports at least 50,000 currently loaded classes as informational context. Frameworks differ in"
                        + " runtime class generation. Lifetime unloads neither prove this population is healthy nor"
                        + " establish a leak; class unloading is optional when defining loaders become reclaimable.",
                "Compare class counts and Metaspace over representative workloads; investigate classloader retention"
                        + " only if growth is unexpected.",
...
final class CommittedFootprintNearContainerLimitRule extends AbstractMemoryRule {
    ...
    CommittedFootprintNearContainerLimitRule() {
        super(new MemoryRuleDefinition(
                ...
                "MEDIUM",
                "Estimates the JVM's configured memory envelope (maximum heap, currently committed non-heap such as"
                        + " Metaspace and code cache, direct-buffer capacity, and approximate thread-stack reservations)"
                        + " against the detected container limit. Using maximum rather than currently committed heap"
                        + " exposes configurations that leave too little native headroom before the heap grows. The"
                        + " estimate is conservative but incomplete: it excludes GC structures, JIT working memory, native"
                        + " libraries, and non-NIO native allocations.",
                "Review configured capacity against measured container usage and native headroom before changing"
                        + " limits. This mixed reservation estimate is neither RSS nor total committed memory.",
        ...
        if (memory.heapMax() >= limit) {
            return violation(
                    ...
                    "Maximum heap " + MemoryFormat.bytes(memory.heapMax())
                            ...
                            + "; the heap configuration alone leaves no native headroom if fully realized. This is not"
                            + " measured residency.");
        }
    ...
    HighGcOverheadRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        MemoryCategory.GC_CONFIGURATION,
                        "MEDIUM",
                        "Compares total time spent in garbage collection since JVM start against the JVM uptime. A"
                                + " high lifetime ratio is a classic sign of an undersized heap or an excessive allocation"
                                + " rate. This is a cumulative average and can be skewed by a one-off startup spike, so"
                                + " corroborate with live GC metrics.",
                        "Increase the heap (-Xmx/-XX:MaxRAMPercentage), reduce the allocation rate, or review the"
                                + " collector choice if GC consistently consumes this much time.",
                        "https://docs.oracle.com/en/java/javase/21/gctuning/factors-affecting-garbage-collection-performance.html"));
    }
...
final class UnequalInitialAndMaxHeapRule extends AbstractMemoryRule {
    ...
    UnequalInitialAndMaxHeapRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.GC_CONFIGURATION,
                "INFO",
                "For low-latency collectors (ZGC, Shenandoah), a smaller -Xms than -Xmx makes the JVM grow and"
                        + " re-commit the heap on demand, which can add latency and commit/uncommit churn. Equal -Xms and"
                        + " -Xmx keep the heap fully committed for steady-state, latency-sensitive services. When -Xms is"
                        + " unset, the JVM's reported initial heap size (ergonomic default, typically ~1/64 of RAM) is"
                        + " used as the effective -Xms.",
                "For latency-sensitive services using ZGC or Shenandoah, set -Xms equal to -Xmx so the heap is fully"
                        + " committed up front; also consider -XX:+AlwaysPreTouch to touch every heap page at startup and"
                        + " avoid OS demand-paging latency during warmup.",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (!lowLatency) {
            return inapplicable("The active collector is not a low-latency collector (ZGC/Shenandoah).");
        }
...
final class PendingFinalizationBacklogRule extends AbstractMemoryRule {
    ...
    PendingFinalizationBacklogRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.HEAP_PRESSURE,
                "LOW",
                "Flags a large backlog of objects pending finalization. The finalizer thread cannot keep up, so these"
                        + " objects (and any native resources they hold) are retained longer than expected. Finalization"
                        + " is deprecated for removal (JEP 421); a backlog usually points to legacy finalizers.",
                "Replace finalizers with try-with-resources, java.lang.ref.Cleaner, or explicit close() methods, and"
...
final class UnboundedMetaspaceInContainerRule extends AbstractMemoryRule {
    ...
    UnboundedMetaspaceInContainerRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.MEMORY_POOLS,
                "LOW",
                "Notes sizable Metaspace with an undefined MXBean maximum inside a detected memory-limited container."
                        + " An undefined maximum is not proof of unlimited capacity, a missing effective VM option, or an"
                        + " impending OOM kill.",
                "Review class-metadata growth and total native/container usage. Consider a deliberate MaxMetaspaceSize"
                        + " only with workload evidence; a cap can cause Metaspace OOM and does not guarantee a graceful"
                        + " failure.",
                "https://docs.oracle.com/en/java/javase/21/vm/class-metadata.html"));
    }
...
final class ClassLoadingChurnRule extends AbstractMemoryRule {
    ...
    ClassLoadingChurnRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.CLASS_LOADING,
                "INFO",
                "Reports a large lifetime class-unload count or lifetime-average rate. These counters can reflect a"
                        + " past burst or redeployment and do not establish sustained recent churn.",
                "Compare recent class-loading observations under representative load before changing dynamic"
                        + " generation, caches, or classloader lifecycles.",
                "https://docs.oracle.com/en/java/javase/21/troubleshoot/troubleshoot-class-loading.html"));
    }
    ...
    RecentGcOverheadRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        MemoryCategory.GC_CONFIGURATION,
                        "MEDIUM",
                        "Compares approximate collection elapsed-time deltas with the interval between valid scans."
                                + " Known concurrent-cycle timers are excluded to avoid cycle/pause double-counting."
                                + " Invalid/reset counters or changed collector identities require a new baseline. A"
                                + " collection crossing a sample boundary can distort a short window; this is not CPU"
                                + " utilization or an exact pause percentage. Histogram request intervals are excluded.",
                        "Re-run the scan after a representative workload; if recent GC overhead stays high, increase"
                                + " the heap (-Xmx/-XX:MaxRAMPercentage), reduce the allocation rate, or review the"
                                + " collector choice.",
...
final class CompressedClassSpaceRule extends AbstractMemoryRule {
    ...
    CompressedClassSpaceRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.MEMORY_POOLS,
                "MEDIUM",
                "Flags Compressed Class Space at 85% of its reported maximum. Compressed class pointers are distinct"
                        + " from compressed ordinary object pointers; availability and capacity depend on JVM version and"
                        + " configuration. Exhaustion can cause OutOfMemoryError: Compressed class space.",
                "Review class generation and the effective class-space and Metaspace limits before changing"
                        + " -XX:CompressedClassSpaceSize; account for the JVM's object-header mode and native headroom.",
                "https://docs.oracle.com/en/java/javase/21/vm/class-metadata.html"));
    }
...
final class ContainerMemoryPressureRule extends AbstractMemoryRule {
    ...
    ContainerMemoryPressureRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.NATIVE_MEMORY,
                "HIGH",
                "Reads the current cgroup memory usage (memory.current for cgroup v2, memory.usage_in_bytes for v1)"
                        + " and compares its working set against the detected container memory limit. When memory.stat"
                        + " exposes inactive file cache, the rule subtracts that reclaimable portion before evaluating"
                        + " pressure. A working set near the limit leaves little room for the next allocation; if the"
                        + " kernel cannot reclaim enough charged memory, it may OOM-kill a process without invoking JVM"
                        + " OutOfMemoryError handling.",
                "Lower -Xmx/-XX:MaxRAMPercentage, reduce non-heap memory (thread stacks, Metaspace, direct buffers),"
                        + " or raise the container memory limit to restore headroom.",
                "https://docs.oracle.com/en/java/javase/21/troubleshoot/diagnostic-tools.html"));
    }
...
final class SerialGcOnMultiCoreRule extends AbstractMemoryRule {
    ...
    SerialGcOnMultiCoreRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.GC_CONFIGURATION,
                "LOW",
                "Notes Serial GC on a JVM with at least two available processors and roughly 2 GiB of known memory."
                        + " This historical HotSpot server-class threshold is context, not proof that Serial is wrong:"
                        + " small heaps and particular workloads may benefit from it.",
                "Compare measured pause, throughput and footprint requirements before choosing a different collector;"
                        + " keep Serial when it suits the workload.",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        if (!serialGc) {
            return memory.gcCollectorNames().isEmpty() ? missingPass() : noTargets();
        }
        int cpus = context.runtime().availableProcessors();
        if (cpus < 2) {
            return inapplicable("Serial GC is expected on a single-CPU system.");
        }
        ...
        if (effectiveMemoryBytes >= 0 && effectiveMemoryBytes < SERVER_CLASS_MEMORY_THRESHOLD_BYTES) {
            return inapplicable("Serial GC is expected below Oracle's historical 'server-class machine' ergonomics"
...
final class G1FullGcFrequencyRule extends AbstractMemoryRule {
    ...
    G1FullGcFrequencyRule() {
        super(new MemoryRuleDefinition(
                ...
                MemoryCategory.GC_CONFIGURATION,
                "INFO",
                "Reports a G1 Old Generation collection-count increase between comparable scans, excluding their"
                        + " histogram request intervals. A Full GC can result from allocation pressure, explicit GC or"
                        + " another diagnostic request. The counter alone does not identify the cause or prove G1 failed"
                        + " to keep up.",
                "Inspect unified GC logs for the collection cause and pause duration before changing heap size or G1"
                        + " tuning. Do not infer allocation failure from the count alone.",
                "https://docs.oracle.com/en/java/javase/21/gctuning/garbage-first-g1-garbage-collector1.html"));
    }

    @Override
    MemoryEvaluation evaluateRule(MemoryContext context) {
        List<String> collectors = context.memory().gcCollectorNames();
        if (!collectors.isEmpty() && collectors.stream().noneMatch(name -> name.startsWith("G1 "))) {
            return inapplicable("G1 is not an observed active collector.");
        }
        MemoryContext.GcTrend trend = context.gcTrend();
        if (!trend.available()) {
...
final class BufferPoolGrowthWithoutReleaseRule extends AbstractMemoryRule {
    ...
    BufferPoolGrowthWithoutReleaseRule() {
        super(
                new MemoryRuleDefinition(
                        ...
                        MemoryCategory.MEMORY_POOLS,
                        "LOW",
                        "Reports three increases in estimated direct-buffer used bytes across four comparable"
                                + " snapshots. Missing or unknown readings break the streak. Net growth does not establish"
                                + " that no releases occurred between samples or that a leak exists; cache warmup and"
                                + " changing load can produce the same observations. Mapped buffers are excluded. Severity"
                                + " is MEDIUM only when capacity is also near a known effective direct-memory cap.",
                        "Compare representative steady-state load and supported buffer-pool lifecycle metrics before"
                                + " investigating retention. Use library release/close APIs where required, not manual"
                                + " Cleaner calls.",
    ...
    MemoryEvaluation evaluateRule(MemoryContext context) {
        ...
        for (MemoryContext.BufferPoolSnapshot pool : pools) {
            ...
            details.add("'" + pool.name() + "' buffer pool usage increased on " + streak
                    + " consecutive scan intervals (now " + MemoryFormat.bytes(pool.used())
                    + "); net growth does not prove missing releases or a leak. Confirm comparable workload"
                    + " conditions.");
        }
        if (details.isEmpty()) {
            return pass();
    ...
    OldGenerationTrendingUpwardRule() {
        super(new MemoryRuleDefinition(
                ...
                "Reports three increases in old-generation occupancy across four comparable post-histogram snapshots."
                        ...
                        + " occupancy is not retained size, and normal warmup or changing load can produce the same"
                        + " trend.",
                "Compare observations under stable load and confirmed collector-specific reclamation before"
                        + " investigating a leak. Use an explicitly requested heap/JFR investigation only when evidence"
                        + " warrants its cost.",
                "https://docs.oracle.com/en/java/javase/21/troubleshoot/troubleshooting-memory-leaks.html"));
    }
