 ...
 * or transport API, so both the Spring Boot and Quarkus adapters can implement them. The ban includes
 * both JSON libraries — Spring Boot 4 ships Jackson 3 ({@code tools.jackson.*}) while Quarkus ships
 * Jackson 2 ({@code com.fasterxml.jackson.*}), incompatible artifact <em>and</em> package — so SPI
 * signatures carry already-parsed neutral DTOs and each adapter owns its own JSON binding. Neutral
 * {@code jakarta.*} contracts ({@code jakarta.persistence}, {@code jakarta.sql}) and Micrometer stay
 * allowed deliberately; only the framework/transport/JSON packages below are banned.
 */
