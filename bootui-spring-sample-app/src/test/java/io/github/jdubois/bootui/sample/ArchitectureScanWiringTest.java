...
/**
 * Real-wiring guard for the Architecture advisor after its extraction into {@code bootui-engine}.
 *
 * <p>The shared conformance suite only exercises {@code GET /bootui/api/architecture}, which returns the
 ...
 * sample app and drives the autoconfigured {@link ArchitectureScanner} bean through a real
 * {@code POST}-equivalent {@code scan()}, asserting it resolves the sample app's base package via the
 * {@code SpringBasePackageProvider -> AutoConfigurationPackages} seam and actually imports its classes.
 */
@SpringBootTest(
        ...
        properties = {
            ...
            "bootui.overrides-file=target/bootui-architecture-wiring-overrides.properties"
        })
class ArchitectureScanWiringTest {

    @Autowired
    ArchitectureScanner architectureScanner;

    @Test
    void scanResolvesSampleBasePackageAndAnalysesClasses() {
        ArchitectureReport report = architectureScanner.scan();
