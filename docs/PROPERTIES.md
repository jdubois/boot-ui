...
| `bootui.activity.*`                                                          | Spring only                | Quarkus doesn't read these tunables: its trace-id-based signal correlation and reduced, trace-id-only profile drill-down (`docs/QUARKUS-SUPPORT.md`) use fixed defaults instead. |
