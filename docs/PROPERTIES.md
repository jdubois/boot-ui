...
| `bootui.trust-container-gateway` | `OFF`                                   | One-flag opt-in to trust the auto-detected container default gateway as a single `/32`, so BootUI can be reached inside a container with a published port (host→container traffic is SNAT'd to the gateway) without knowing the subnet or setting a broad `bootui.trusted-proxies` CIDR. `OFF` (default, fail closed) never trusts it; `AUTO` auto-detects and trusts the gateway `/32` only when running inside a container; `ON` trusts a detected gateway even if container heuristics are inconclusive. Relaxes only the source-address check — the `Host` allow-list (DNS-rebinding) and cross-site write (CSRF) protections stay in force. Note: with the common `-p 8080:8080` bind, LAN clients reaching the published port are also SNAT'd to the gateway; use `-p 127.0.0.1:8080:8080` for strict loopback equivalence. |
