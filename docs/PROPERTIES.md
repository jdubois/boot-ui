...
| `bootui.trust-container-gateway` | `AUTO`                                  | Trust the auto-detected container default gateway as a single `/32` so BootUI works out of the box inside a container with a published port (host→container traffic is SNAT'd to the gateway) without a broad `bootui.trusted-proxies` CIDR. `AUTO` trusts it only when running inside a container; `ON` trusts a detected gateway even if container heuristics are inconclusive; `OFF` never trusts it. Relaxes only the source-address check — the `Host` allow-list (DNS-rebinding) and cross-site write (CSRF) protections stay in force. Note: with the common `-p 8080:8080` bind, LAN clients reaching the published port are also SNAT'd to the gateway; use `-p 127.0.0.1:8080:8080` (or `OFF`) for strict loopback equivalence. |
