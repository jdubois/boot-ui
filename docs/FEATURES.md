...
shared trace id; without tracing it falls back to HTTP-request anchoring (path and time window) and then thread plus
time-window heuristics, and the panel makes that confidence explicit with a per-request badge (Trace-linked, Request, or
Heuristic) and a hint when no trace id is observed. Signals that cannot be tied to a request are surfaced in a separate
"Unattributed signals" list rather than being silently dropped. Selecting a request reveals its chronological timeline
