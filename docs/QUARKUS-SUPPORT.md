...
sender side of the primary demo scenario (a Quarkus app forwarding to a Spring app) could not work at all. The
**receiving** endpoint (`POST /bootui/api/activity/forward`), however, exists only on the Spring adapter in this
version — a Quarkus instance can be configured as a forwarding sender but not (yet) as a receiver; see
[SPECIFICATION.md §5.14.2](SPECIFICATION.md) and [PROPERTIES.md](PROPERTIES.md) for the full design, config keys, and
security posture.
