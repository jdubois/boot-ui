...
(→ `quarkus-scheduler`) · `HTTP Clients` (**Implemented** — → MicroProfile `@RegisterRestClient` interfaces scanned from
the build-time Jandex index by a `registerHttpClients` build step + `@Recorder` into a synthetic bean, with
`quarkus.rest-client.*` and `<key>/mp-rest/*` settings resolved per request through SmallRye Config; the same shared DTO
Spring fills from HTTP Interface, OpenFeign and builder bean definitions) · `Architecture` advisor (shared ArchUnit registry; generic rules run unchanged, Spring-only
...
| HTTP Clients        | equiv       | Adapt   | HTTP client registry mapper      | `HttpClientProvider` → `@RegisterRestClient` + SmallRye Config |
