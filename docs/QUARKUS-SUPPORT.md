...
| `HTTP Exchanges`      | **Implemented** — buffer exchanges via a Vert.x filter instead of Actuator's repository. **Correlation-identifier capture is at full parity** and required no Quarkus-specific capture: the Vert.x filter already hands the shared `HttpExchangesService` the inbound headers, and that service performs extraction, bounding, masking and lookup-identity derivation for every runtime. Additional header names bind through `bootui.http-exchanges.correlation-id-headers` on a `CorrelationIdSettings` CDI producer, the same key the Spring adapter uses, with refused names reported once at startup |
