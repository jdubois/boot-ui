...
**HTTP forwarding (`bootui.activity.forwarding.*`) is implemented as a sender on Quarkus too, wired identically to
Spring.** `BootUiEngineProducer` gains a second `@Produces ActivityForwardingSettings` reading the
`bootui.activity.forwarding.*` keys live from MicroProfile `Config` (same defaults as Spring's properties, including
resolving `instanceId` the same way `activityPersistenceSettings` does), and `activityStore(...)` calls the same
3-arg `ActivityStoreFactory.create(persistenceSettings, forwardingSettings, dataSources)` overload Spring's
configuration uses — building a `BufferedActivityStore` wrapping the shared engine `HttpActivityStore` when
forwarding is enabled, exactly like the JDBC path builds one wrapping `JdbcActivityStore`. `QuarkusActivityCapture`'s
`onStart` gates on `persistenceSettings.enabled()` **or** `forwardingSettings.enabled()` (previously persistence-only),
so a Quarkus instance configured purely as a forwarding sender still starts its capture poller — without this the
sender side of the primary demo scenario (a Quarkus app forwarding to a receiver) could not work at all. The
**receiving** endpoint (`POST /bootui/api/activity/forward`), however, exists only on the Spring host adapter in this
version — a Quarkus instance can be configured as a forwarding sender but not (yet) as a receiver. A `peer-base-url`
can point at either kind of receiver with zero code changes on the sender side (just a different URL): a Spring host
adapter acting as its own receiver, or the standalone **BootUI Activity Console** (`bootui-activity-console`, port
8079 by default) — a dedicated Spring WebFlux app built exactly for this purpose that aggregates forwarded activity
from every BootUI-enabled sender (Spring or Quarkus) into one cross-instance Live Activity feed; the Quarkus sample
app is wired to the Console by default. See [SPECIFICATION.md §5.14.2](SPECIFICATION.md) and
[PROPERTIES.md](PROPERTIES.md) for the full design, config keys, and security posture.
...
| Live Activity       | **done**    | Rebuild | Activity model                   | `RequestCaptureSource` → Vert.x; OTel trace-id correlation + trace-id-only profile drill-down; optional JDBC persistence backend via `QuarkusActivityCapture` (unconditional producers, identical to Spring) |
