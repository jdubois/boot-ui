...
/**
 * Ground-truth parity check that the Quarkus Jackson 2 ({@code com.fasterxml.jackson.*})
 * {@link GitHubApiClient} parses GitHub REST responses identically to the Spring adapter's Jackson 3 client.
 *
 * <p>Mirrors the Spring {@code GitHubApiClientTests}: a {@link HttpServer} on loopback serves the same canned
 * GitHub JSON, the client is pointed at it with {@code localhost} on the host allow-list, and the resulting
 * {@link GitHubDashboardReport} is asserted field-by-field. This pins the one place the two adapters' clients
 * diverge ({@code JsonNode.asText()} vs Jackson 3's {@code asString()}, the {@code .properties()} rate-limit
 * iteration) so a Jackson-2 regression surfaces here rather than in the browser.</p>
 */
class GitHubApiClientTest {
    ...
    void refreshLoadsRepositoryMetricsAndDynamicQuotaResources() throws Exception {
        ...
        // No-leak guard: neither the bearer token nor the Copilot signed download URL may appear anywhere in
        // the serialized report the browser receives.
        String serialized = new ObjectMapper().writeValueAsString(report);
        assertThat(serialized).doesNotContain("local-token");
        assertThat(serialized).doesNotContain("signed.example");
    }
    ...
    void rejectsRefreshWhenApiHostIsNotAllowed() throws Exception {
        ...
        QuarkusGitHubSettings settings =
                new QuarkusGitHubSettings(Duration.ofSeconds(2), 10, 25, 20, 10, 17, 50, List.of("api.github.com"));
        GitHubApiClient client = new GitHubApiClient(
                settings,
                HttpClient.newHttpClient(),
                new ObjectMapper(),
                timeout -> new GitHubTokenProvider.Token("local-token", "test-token"));
    ...
    private GitHubApiClient client(String token) {
        QuarkusGitHubSettings settings =
                new QuarkusGitHubSettings(Duration.ofSeconds(2), 10, 25, 20, 10, 17, 50, List.of("localhost"));
