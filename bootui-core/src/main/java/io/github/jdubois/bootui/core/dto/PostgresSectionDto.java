 ...
 * this view" must never look like "this subsystem has nothing to show". An empty {@code AVAILABLE} section
 * is the only way the panel says "read, and the server reported nothing".</p>
 *
 * @param id the stable section id ({@code vital-signs}, {@code sessions}, {@code statements},
 *     {@code indexes}, {@code tables}, {@code vacuum}, {@code replication}, {@code settings})
...
public record PostgresSectionDto(
        String id, String title, String status, String reason, String hint, int rowCount, boolean truncated) {}
